          ...
          <div className="card p-4 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                <button onClick={() => setSelectedCategory('')} className={`block w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${!selectedCategory ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}>
                  All Categories
                </button>
                {categories.map(cat => {
                  const count = products.filter(p => p.category === cat.id).length
                  return (
                    <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`block w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${selectedCategory === cat.id ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[0, 3, 4, 4.5].map(r => (
                  <button key={r} onClick={() => setMinRating(r)} className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm transition-colors ${minRating === r ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'}`}>
                    {r === 0 ? 'All Ratings' : (
                      <>
        ...
        <div className="flex-1">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <div className="flex gap-1">
              <button onClick={() => setViewMode('grid')} className={`p-3 rounded-lg ${viewMode === 'grid' ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-gray-500'}`} aria-label="Grid view">
                <Grid className="w-5 h-5" />
              </button>
              <button onClick={() => setViewMode('list')} className={`p-3 rounded-lg ${viewMode === 'list' ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-gray-500'}`} aria-label="List view">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button onClick={clearAllFilters} className="text-amber-600 hover:text-amber-700 mt-2 font-semibold">
                Clear all filters
              </button>
            </div>
          ) : viewMode === 'grid' ? (
                  ...
                  <div className="p-4 flex-1">
                    <div className="flex justify-between">
                      <div>
                        <p className="text-sm text-amber-600 font-medium">{product.seller}</p>
                        <Link to={`/product/${product.id}`}>
                          <h3 className="font-semibold text-gray-900 text-lg hover:text-amber-600 transition-colors">{product.name}</h3>
                        </Link>
                      </div>
                      <span className="text-2xl font-bold text-green-600">{formatMoney(product.price)}</span>
                    </div>
